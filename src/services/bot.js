...
async function handleLottieCommand(sock, jid, m, config, args) {
  ...
  const media = await downloadMessageMedia(payload);
  const rawSelection = args[0] || 'secondary';
  const normalizedSelection = rawSelection.toLowerCase();
  const resolvedSelection =
    normalizedSelection === 'main' || normalizedSelection === 'secondary'
      ? normalizedSelection
      : rawSelection;
  const jsonRelativePath = resolveLottieJsonPath(config, resolvedSelection);
